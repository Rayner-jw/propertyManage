        ...
        <el-table :data="tableData" align="center" width="100%">
          ...
          <el-table-column align="center" label="年度" min-width="100">
            <template slot-scope="scope">
              <span>{{ formatDate(new Date(scope.row.theYear),'yyyy') }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" label="发起时间" min-width="100">
            <template slot-scope="scope">
              <span>{{ formatDate(new Date(scope.row.theYear),'yyyy-MM-dd HH:mm:ss') }}</span>
            </template>
          </el-table-column>
    ...
    <el-dialog title="发起" :visible.sync="dialogTableVisible">
      <el-form class="ruleForm" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="80px">
          ...
          <el-date-picker
  		  ...
  		  <el-form-item label="">
          <el-button size="small" type="primary" @click="submitForm('ruleForm')">发起</el-button>
  		  </el-form-item>
  		</el-form>
    </el-dialog>
  </div>
  ...
  methods:{
    submitForm (formName) {
      ...
      this.ruleForm.createAt = formatDate(new Date(),'yyyy-MM-dd HH:mm:ss');
  		this.$http.put('http://localhost:3000/damin/paymentApi', this.ruleForm).then(
  			function(data){
          var type = 'success';
          if(typeof(res.body.type) != 'undefined'){
            type = res.body.type;
          }
          Message({
            type:type,
