<template>
  <div class="receiveMsgs">
    <span class="title">已收消息</span>
    <div class="main">
      ...
      <el-table :data="tableData" highlight-current-row @row-click="handleCurrent" align="center" width="100%">
        ...
        <el-table-column align="center" prop="sendFrom" label="来自" min-width="100"></el-table-column>
    ...
    <el-dialog center title="消息内容" :before-close="handleClose" :visible.sync="readBox">
      <div class="readDiv">
          <h3>
...
export default {
  ...
  methods:{
    ...
    pageEvent (a){
      ...
      this.$http.post('http://localhost:3000/admin/message',para).then(
        function(data){
    ...
    handleCurrent(row) {
      ...
      //未读则更新 已读
      if(row.isread == 0){
        this.$http.put('http://localhost:3000/admin/message',{messageId: row.messageId}).then(
          function(data){
            console.log(data);
          },
          function(res){
...
.receiveMsgs .readMain > span{
  display: inline-block;
  margin-right: 20px;
}
.receiveMsgs .readDiv h3{
  margin-top: 0;
}
.receiveMsgs .readDiv p{
  text-align: center;
  margin-bottom: 100px;
}
.receiveMsgs .el-button--small{
  padding: 7px 15px;
  margin-right: 10px;
}
.receiveMsgs .el-breadcrumb{
  display: inline-block;
}
.receiveMsgs .el-breadcrumb__inner{
  font-weight: normal;
  color: #606266;
}
</style>
