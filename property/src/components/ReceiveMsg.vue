...
export default {
  ...
  methods:{
    ...
    handleCurrent(row) {
      ...
      //未读则更新 已读
      if(row.isread == 0){
        this.$http.get('http://localhost:3000/user/message?messageId=' + row.messageId).then(
          function(data){
            console.log(data);
          },
          function(res){
...
.receiveMsg .readMain > span{
  display: inline-block;
  margin-right: 20px;
}
.receiveMsg .readDiv h3{
  margin-top: 0;
}
.receiveMsg .readDiv p{
  text-align: center;
  margin-bottom: 100px;
}
.receiveMsg .el-button--small{
  padding: 7px 15px;
  margin-right: 10px;
}
.receiveMsg .el-breadcrumb{
  display: inline-block;
}
.receiveMsg .el-breadcrumb__inner{
  font-weight: normal;
  color: #606266;
}
</style>
